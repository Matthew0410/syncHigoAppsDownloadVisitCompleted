<?php
	...
	header('Content-Disposition: attachment;filename="'.$merchant_name.' - '.date('Y-m-d', $post_date).'.xls"');
	...
	function write_excel($phpexcel, $merchant_name, $arr_data)
	{
		...
		$next_row = $row + 1;
		$phpexcel->getActiveSheet()->mergeCells("B{$row}:I{$next_row}");
		...
		foreach ($arr_data as $data)
		{
			...
			$phpexcel->getActiveSheet()->SetCellValue("B{$row}", $data->time);
